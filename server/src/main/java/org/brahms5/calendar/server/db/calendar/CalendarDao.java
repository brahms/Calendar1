...
public class CalendarDao extends ADao implements ICalendarDao {
	...
	public static final String SQL_UPDATE = 
			"UPDATE "+ TABLE_NAME + " SET blob= ? WHERE username= ?;";
	public static final String SQL_SELECT = 
			"SELECT " + COLUMNS_CALENDAR + " FROM " + TABLE_NAME + " WHERE username = ?;";
	
	public static final String SQL_SELECT_ALL = "SELECT " + COLUMNS_CALENDAR + " FROM " + TABLE_NAME + ";";
	...
	public void insert(Calendar calendar)
	{
		...
		insert.update(SQL_INSERT, new Object[]{calendar.getUser().getName(), SerializationUtils.serialize(calendar)});
	}
	@Override
	public Calendar create(User user) {
		...
		else 
		{
			...
			JdbcTemplate insert = new JdbcTemplate(dataSource);
			insert.update(SQL_INSERT, new Object[]{user.getName(), SerializationUtils.serialize(calendar)});
		}
		return calendar;
	}
	...
	public void update(Calendar calendar) {
		...
		update.update(SQL_UPDATE, new Object[]{SerializationUtils.serialize(calendar), calendar.getUser().getName()});
	...
	public Calendar select(User user) {
		...
		List<Calendar> list =  select.query(SQL_SELECT, new Object[]{user.getName()}, new CalendarRowMapper());
		if (list.isEmpty())
		{
	...
	public List<Calendar> selectAll() 
	{
		...
		JdbcTemplate select = new JdbcTemplate(dataSource);
		List<Calendar> list = select.query(SQL_SELECT_ALL, new CalendarRowMapper());
	...
	public Integer count() 
	{
		...
		return count.query("SELECT count(*) FROM " + TABLE_NAME, new RowMapper<Integer>(){
